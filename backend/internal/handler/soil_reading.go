...
type SoilReadingHandler struct{ service service.SoilReadingService }

func NewSoilReadingHandler(s service.SoilReadingService) *SoilReadingHandler {
	return &SoilReadingHandler{service: s}
}

func (h *SoilReadingHandler) Register(group *gin.RouterGroup) {
	resource := group.Group("/readings")
	resource.GET("", h.list)
...
func (h *SoilReadingHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *SoilReadingHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *SoilReadingHandler) create(c *gin.Context) {
	var input dto.CreateSoilReading
...
func (h *SoilReadingHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateSoilReading
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *SoilReadingHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *SoilReadingHandler) remove(c *gin.Context) {
