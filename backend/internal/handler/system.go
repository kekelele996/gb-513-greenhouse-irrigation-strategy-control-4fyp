...
type SystemHandler struct {
	...
	greenhouseZone service.GreenhouseZoneService
	soilReading    service.SoilReadingService
	irrigationPlan service.IrrigationPlanService
	valveExecution service.ValveExecutionService
	db             *gorm.DB
	redis          *redis.Client
}

func NewSystemHandler(security service.SecurityService, greenhouseZone service.GreenhouseZoneService, soilReading service.SoilReadingService, irrigationPlan service.IrrigationPlanService, valveExecution service.ValveExecutionService, db *gorm.DB, redisClient *redis.Client) *SystemHandler {
	return &SystemHandler{security: security, greenhouseZone: greenhouseZone, soilReading: soilReading, irrigationPlan: irrigationPlan, valveExecution: valveExecution, db: db, redis: redisClient}
}
...
func (h *SystemHandler) Overview(c *gin.Context) {
	...
	greenhouseZoneCounts, err := h.greenhouseZone.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["zones"] = greenhouseZoneCounts

	soilReadingCounts, err := h.soilReading.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["readings"] = soilReadingCounts

	irrigationPlanCounts, err := h.irrigationPlan.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["plans"] = irrigationPlanCounts

	valveExecutionCounts, err := h.valveExecution.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["executions"] = valveExecutionCounts
