...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	valveExecutionService := service.NewValveExecutionService(valveExecutionRepository, greenhouseZoneRepository, soilReadingRepository, irrigationPlanRepository, securityService)
